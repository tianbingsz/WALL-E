...
def plot_average_return(keys, inputfile, outputfile):
    ...
    sIdx = data[0].index('Steps')
    steps = np.array([d[sIdx] for d in data[1::]], dtype='float32')
    # plot
    pyplot.plot(steps, returns, linewidth=3.0)
    pyplot.title('average return over steps')
    # pyplot.xscale('log')
    pyplot.xlabel('steps')
